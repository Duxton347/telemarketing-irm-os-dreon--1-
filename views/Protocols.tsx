...
import {
   ...
   Check, Send, ClipboardCheck, ListChecks, MessageCircle, FileText, Download
} from 'lucide-react';
...
const Protocols: React.FC<{ user: UserType }> = ({ user }) => {
   ...
   const handleExportPDF = async () => {
      ...
      try {
         ...
         const title = "Relatório de Protocolos Não Finalizados";
         doc.setFontSize(16);
         doc.text(title, 14, 15);
         doc.setFontSize(10);
         doc.text(`Gerado em: ${new Date().toLocaleString()}`, 14, 22);

         const unfinishedProtocols = filtered.filter(p => p.status !== ProtocolStatus.FECHADO);

         if (unfinishedProtocols.length === 0) {
            alert("Nenhum protocolo não-finalizado para exportar.");
            setIsProcessing(false);
            return;
         }
         ...
         for (const p of unfinishedProtocols) {
            const client = clients.find(c => c.id === p.clientId);
            const { calls } = await dataService.getClientHistory(p.clientId);
            const events = await dataService.getProtocolEvents(p.id);
            ...
            doc.text(`Protocolo #${p.protocolNumber || p.id.substring(0, 8)} - ${p.status} - Prioridade: ${p.priority}`, 14, yPos);
            yPos += 6;

            doc.setFontSize(10);
            doc.setFont("helvetica", "normal");
            if (client) {
               doc.text(`Cliente: ${client.name} | Telefone: ${client.phone}`, 14, yPos);
               yPos += 6;
            }

            doc.setFont("helvetica", "bold");
            doc.text("Relato do Cliente:", 14, yPos);
            yPos += 5;
            doc.setFont("helvetica", "normal");
            const splitDesc = doc.splitTextToSize(p.description || 'Sem descrição', 180);
            doc.text(splitDesc, 14, yPos);
            yPos += (splitDesc.length * 5) + 5;

            // Histórico de Contatos (Calls)
            if (calls.length > 0) {
               autoTable(doc, {
                  startY: yPos,
                  head: [['Data', 'Tipo', 'Atendente']],
                  body: calls.map(c => [
                     new Date(c.startTime).toLocaleString(),
                     c.type,
                     operators.find(o => o.id === c.operatorId)?.name || 'N/A'
                  ]),
                  margin: { left: 14 },
                  theme: 'grid',
            ...
            // Histórico de Eventos do Protocolo
            if (events.length > 0) {
               autoTable(doc, {
                  startY: yPos,
                  head: [['Data', 'Tipo de Evento', 'Nota', 'Ator']],
                  body: events.map(e => [
                     new Date(e.createdAt).toLocaleString(),
                     e.eventType === 'status_change' ? 'Status Alterado' : 'Anotação',
                     e.note || '',
                     operators.find(o => o.id === e.actorId)?.name || 'Sistema'
                  ]),
                  margin: { left: 14 },
                  theme: 'grid',
         ...
         doc.save(`protocolos_abertos_${new Date().getTime()}.pdf`);
      } catch (error) {
   ...
   const filtered = protocols.filter(p => {
      ...
      const aFechado = a.status === ProtocolStatus.FECHADO;
      const bFechado = b.status === ProtocolStatus.FECHADO;
      if (aFechado && !bFechado) return 1;
      if (!aFechado && bFechado) return -1;
   ...
   return (
      <div className="space-y-6 pb-20 animate-in fade-in duration-500">
         <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex gap-3">
               <button
                  ...
                  className="bg-orange-600 text-white px-6 py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-xl hover:bg-orange-700 active:scale-95 flex items-center gap-2 disabled:opacity-50 transition-all"
                  title="Exportar pdf dos protocolos que ainda não foram fechados"
               >
                  {isProcessing ? <Loader2 size={18} className="animate-spin" /> : <Download size={18} />}
                  Exportar PDF
               </button>
               <button onClick={() => setIsNewModalOpen(true)} className="bg-blue-600 text-white px-8 py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-xl hover:bg-blue-700 active:scale-95 flex items-center gap-2">
         ...
         <div className="grid grid-cols-1 gap-4">
            {filtered.map(p => {
               const isFechado = p.status === ProtocolStatus.FECHADO;
               const isAberto = p.status === ProtocolStatus.ABERTO || p.status === ProtocolStatus.REABERTO || p.status === ProtocolStatus.EM_ANDAMENTO;
               const isPending = p.status === ProtocolStatus.RESOLVIDO_PENDENTE;

               const client = clients.find(c => c.id === p.clientId);

               let cardClass = 'bg-white border-slate-100 hover:border-blue-400';
               let badgeClass = 'bg-blue-600 text-white';

               if (isAberto) {
                  cardClass = 'bg-red-50/50 border-red-500 shadow-xl shadow-red-500/10 animate-pulse';
                  badgeClass = 'bg-red-600 text-white shadow-lg';
               } else if (isPending) {
                  cardClass = 'bg-orange-50 border-orange-400 shadow-xl shadow-orange-500/10';
                  badgeClass = 'bg-orange-500 text-white';
               } else if (isFechado) {
                  cardClass = 'bg-emerald-50/30 border-emerald-300 opacity-80';
                  ...
                  >
                     <div className="flex-1 space-y-2 w-full">
                        <div className="flex items-center gap-3 mb-2">
                           <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest ${badgeClass}`}>{p.status}</span>
                           <span className="text-[10px] font-black text-slate-300">#{p.protocolNumber || p.id.substring(0, 8)}</span>
                        </div>
                        <h3 className="text-xl font-black text-slate-800 leading-tight uppercase tracking-tight">{client?.name || 'Cliente Desconhecido'}</h3>
                        <p className="text-sm font-bold text-slate-500 uppercase tracking-tight line-clamp-1">{p.title}</p>
                     ...
                     <div className="flex items-center gap-6 shrink-0">
                        {p.ownerOperatorId === user.id && (p.status === ProtocolStatus.ABERTO || p.status === ProtocolStatus.REABERTO) && (

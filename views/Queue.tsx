...
const Queue: React.FC<QueueProps> = ({ user }) => {
  ...
  const confirmRescheduleSkip = async (interval: '1d' | '2d' | '1w' | '1m') => {
    ...
    if (interval === '2d') date.setDate(date.getDate() + 2);
    if (interval === '1w') date.setDate(date.getDate() + 7);
    if (interval === '1m') date.setMonth(date.getMonth() + 1);

    // Default time: keep same time or set to 09:00? User didn't specify, but keeping "next day" implies full day bucket.
    // However, tasks usually need a specific time for ordering. Let's set to 09:00 AM.
    date.setHours(9, 0, 0, 0);

    try {
      await dataService.updateTask(currentTask.id, {
        status: 'pending', // Keep pending so it comes back later
        scheduledFor: date.toISOString(),
        scheduleReason: `Repique: ${skipReasonSelected}`,
        approvalStatus: 'PENDING', // Requires Admin approval
        skipReason: skipReasonSelected // Log why it was bumped
      });
      await dataService.logOperatorEvent(user.id, OperatorEventType.PULAR_ATENDIMENTO, currentTask.id, `${skipReasonSelected} (Reagendado para ${date.toLocaleDateString()})`);
      await fetchQueue();
    } catch (e) {
      alert("Erro ao reagendar contato.");
    } finally {
      setIsProcessing(false);
  ...
  const handleSubmitReport = async () => {
    ...
    try {
      ...
      if (scheduleData.isScheduling) {
        // Option A: Complete current task and create new one
        // Option B: Just update the scheduledFor of current task?
        // Usually, the "task" loop suggests completing the "attempt" and if needed, scheduling a NEW attempt.
        // However, to keep it simple and track the "Lead", we can just Reschedule the SAME task if the goal wasn't met.
        // BUT, `saveCall` links to `taskId`. If we keep using the same task ID for multiple calls, it's fine.
        // Let's Reschedule the CURRENT task.
        ...
        await dataService.updateTask(currentTask.id, {
          ...
          targetCallType: currentTask.type // Same type by default, or could allow changing
        });
      } else {
        await dataService.updateTask(currentTask.id, { status: 'completed' });
      }
        ...
        <div className="fixed inset-0 z-[160] bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-[48px] shadow-2xl overflow-hidden animate-in zoom-in duration-300">
            <div className="bg-orange-600 p-10 text-white text-center">
              ...
              <p className="text-orange-100 font-bold mt-2">Quando devemos tentar novamente?</p>
            </div>
            <div className="p-10 grid grid-cols-2 gap-4">
              <button onClick={() => confirmRescheduleSkip('1d')} className="p-6 bg-slate-50 border-2 border-slate-100 rounded-[32px] hover:border-orange-500 hover:bg-orange-50 transition-all group">
                <span className="block text-2xl font-black text-slate-800 group-hover:text-orange-600 mb-1">1 Dia</span>
                <span className="text-[10px] font-bold uppercase text-slate-400">Amanhã</span>
              </button>
              <button onClick={() => confirmRescheduleSkip('2d')} className="p-6 bg-slate-50 border-2 border-slate-100 rounded-[32px] hover:border-orange-500 hover:bg-orange-50 transition-all group">
                <span className="block text-2xl font-black text-slate-800 group-hover:text-orange-600 mb-1">2 Dias</span>
                <span className="text-[10px] font-bold uppercase text-slate-400">Depois de amanhã</span>
              </button>
              <button onClick={() => confirmRescheduleSkip('1w')} className="p-6 bg-slate-50 border-2 border-slate-100 rounded-[32px] hover:border-orange-500 hover:bg-orange-50 transition-all group">
                <span className="block text-2xl font-black text-slate-800 group-hover:text-orange-600 mb-1">1 Semana</span>
                <span className="text-[10px] font-bold uppercase text-slate-400">Próxima semana</span>
              </button>
              <button onClick={() => confirmRescheduleSkip('1m')} className="p-6 bg-slate-50 border-2 border-slate-100 rounded-[32px] hover:border-orange-500 hover:bg-orange-50 transition-all group">
                <span className="block text-2xl font-black text-slate-800 group-hover:text-orange-600 mb-1">1 Mês</span>
                <span className="text-[10px] font-bold uppercase text-slate-400">Próximo mês</span>
              </button>
            </div>
            <div className="pb-10 px-10">
              <button onClick={() => setIsRescheduleModalOpen(false)} className="w-full text-center text-slate-400 font-bold text-xs hover:text-red-500 uppercase tracking-widest">Cancelar (Voltar)</button>
            </div>
          </div>
        </div>
      )}

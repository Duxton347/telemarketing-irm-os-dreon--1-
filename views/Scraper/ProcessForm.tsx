...
export const ProcessForm: React.FC<ProcessFormProps> = ({ onClose, onSuccess, user }) => {
    ...
    const handleVerify = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        try {
            // 1. Verify Location via Backend
            const result = await scraperService.verifyLocation(locationInput);
            setResolvedLocation(result);

            // 2. Generate Map URL (Using Embed API for simplicity & no-key requirement on frontend if possible)
            // Actually, Embed API requires a key too for new accounts, but 'maps.google.com/maps?q=...' iframe often works freely.
            ...
            alert(`Erro ao verificar local: ${error.message}`);
        } finally {
            setIsLoading(false);
        }
    };

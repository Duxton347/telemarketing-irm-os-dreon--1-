...
export const ProcessForm: React.FC<ProcessFormProps> = ({ onClose, onSuccess, user }) => {
    ...
    const handleVerify = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const url = import.meta.env.PROD ? `https://api.codetabs.com/v1/proxy?quest=${encodeURIComponent(googleUrl)}` : `/google-proxy/maps/api/geocode/json?address=${encodeURIComponent(locationInput)}&key=${GOOGLE_MAPS_KEY}`;
